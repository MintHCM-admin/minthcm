            ...
            <template v-for="newsItem in wall.newsList" :key="newsItem.id">
                <div class="mint-news-card">
                    ...
                    <div class="empty-space">
                        ...
                        <div v-if="wall.getEmojiForReaction()" class="reaction-strip">
                            <span v-for="(emoji, index) in wall.getEmojiForReaction()" :key="index">
                                {{ emoji }}
                                    <span v-for="(reaction, index2) in newsItem.reactions" :key="index">
                                        <span v-if="reaction.type == index" >
                                            {{ reaction.user_id }}
                                        </span>
                                    </span>
                            </span>
                        ...
                        <div v-for="(reaction, index2) in newsItem.reactions" :key="reaction.id">
                            <div v-if="wall.getEmojiForReaction(reaction.type)" class="reaction-strip">
                                <span v-for="(emoji, index) in wall.getEmojiForReaction(reaction.type)" :key="index">
                                    {{ emoji }}
                                <span v-for="(reactionInner, index3) in newsItem.reactions" :key="reactionInner.id">
                                    <span v-if="newsItem.selectedReactions.includes(reactionInner.type)">
                                        {{ reactionInner.user_id }}
                                    </span>
                                </span>
                            </span>
                            <span v-if="newsItem.reactionsCount > 0" class="reaction-count" style="margin-left: 10px; color:#145d7b; font-weight: bold;">
...
const addReaction = (newsId: string, reactionType: string) => {
    ...
    console.log("selectedReactions перед includes():", newsItem.selectedReactions);
