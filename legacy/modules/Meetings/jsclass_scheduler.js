...
GLOBAL_REGISTRY.focus.fields.duration_hours=document.forms[form_name].duration_hours.value;GLOBAL_REGISTRY.focus.fields.duration_minutes=document.forms[form_name].duration_minutes.value;GLOBAL_REGISTRY.focus.fields.datetime_start=SugarDateTime.mysql2jsDateTime(GLOBAL_REGISTRY.focus.fields.date_start,GLOBAL_REGISTRY.focus.fields.time_start);if(GLOBAL_REGISTRY['users_rows']!=undefined){GLOBAL_REGISTRY['users_rows']=[];}
if(GLOBAL_REGISTRY['users_rows_ids']!=undefined){GLOBAL_REGISTRY['users_rows_ids']={};}
GLOBAL_REGISTRY.scheduler_attendees_obj.init();GLOBAL_REGISTRY.scheduler_attendees_obj.display();}
...
html+='<td>&nbsp;</td>';html+='</tr>';if(typeof(GLOBAL_REGISTRY)=='undefined'){return;}
GLOBAL_REGISTRY['schedulerTable']=html;html+='</table>';if(this.parentNode.childNodes.length<1)
this.parentNode.innerHTML+='<div class="schedulerDiv">'+html+'</div>';else
this.parentNode.childNodes[0].innerHTML=html;var thetable="schedulerTable";if((typeof(GLOBAL_REGISTRY.focus.users_arr)=='undefined'||GLOBAL_REGISTRY.focus.users_arr.length==0)&&document.forms[form_name].record.value==''&&typeof(GLOBAL_REGISTRY.FIRST_REMOVE)=='undefined'){GLOBAL_REGISTRY.focus.users_arr=[GLOBAL_REGISTRY.current_user];}
...
SugarWidgetScheduleRow.prototype.display=function(){if(GLOBAL_REGISTRY['users_rows']==undefined){GLOBAL_REGISTRY['users_rows']=[];}
if(GLOBAL_REGISTRY['users_rows_ids']==undefined){GLOBAL_REGISTRY['users_rows_ids']={};}
SUGAR.util.doWhen("document.getElementById('"+this.thetableid+"') != null",function(){var tr;this.thetable=document.getElementById(this.thetableid);if(typeof(this.element)!='undefined'){if(this.element.parentNode!=null)
tr=document.createElement('tr');}else{tr=document.createElement('tr');}
...
+GLOBAL_REGISTRY['meeting_strings']['LBL_REMOVE']+'</a>';this.element=tr;this.element_index=this.thetable.rows.length-1;if(GLOBAL_REGISTRY['users_rows_ids']!=undefined&&!(this.focus_bean.fields.id in GLOBAL_REGISTRY['users_rows_ids'])){GLOBAL_REGISTRY['users_rows_ids'][this.focus_bean.fields.id]=tr.outerHTML;GLOBAL_REGISTRY['users_rows'].push(tr.outerHTML);}
let users_rows_html='';if(GLOBAL_REGISTRY['users_rows']!=undefined){GLOBAL_REGISTRY['users_rows'].forEach(function(row){users_rows_html+=row;});}
this.thetable.innerHTML=GLOBAL_REGISTRY['schedulerTable']+users_rows_html;}},null,this);}
SugarWidgetScheduleRow.deleteRow=function(bean_id){for(var i=0;i<GLOBAL_REGISTRY.focus.users_arr.length;i++){if(GLOBAL_REGISTRY.focus.users_arr[i]['fields']['id']==bean_id){delete GLOBAL_REGISTRY.focus.users_arr_hash[GLOBAL_REGISTRY.focus.users_arr[i]['fields']['id']];GLOBAL_REGISTRY.focus.users_arr.splice(i,1);GLOBAL_REGISTRY.FIRST_REMOVE=true;if(GLOBAL_REGISTRY['users_rows_ids'][bean_id]!=undefined){if($.inArray(GLOBAL_REGISTRY['users_rows_ids'][bean_id],GLOBAL_REGISTRY['users_rows'])!=-1){GLOBAL_REGISTRY['users_rows'].splice($.inArray(GLOBAL_REGISTRY['users_rows_ids'][bean_id],GLOBAL_REGISTRY['users_rows']),1);delete GLOBAL_REGISTRY['users_rows_ids'][bean_id];}}
